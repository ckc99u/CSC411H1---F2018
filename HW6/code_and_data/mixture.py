...
class Params(object):
    """A class which represents the trainable parameters of the mixture model.
        - pi: the mixing proportions, represented as a K-dimensional array. It must be a
            probability distribution, i.e. the entries must be non-negative and sum to 1.
...
class Model(object):
    ...
    def expected_joint_log_probability(self, X, R):
        ...
        log_p_x_given_z = np.dot(X, np.log(self.params.theta).T) + np.dot(1. - X, np.log(1. - self.params.theta).T)
    ...
    def update_pi(self, R):
        ...
        ######################## Your code here #########################
        updated_pi = np.sum(R, axis=0) + self.prior.a_mix - 1
        updated_pi /= np.sum(updated_pi)  # normalization
    ...
    def update_theta(self, X, R):
        ...
        ######################## Your code here #########################
        numer = self.prior.a_pixels - 1 + np.matmul(np.transpose(R), X)
        denom = self.prior.a_pixels + self.prior.b_pixels - 2 + np.sum(R, axis=0)
    ...
    def compute_posterior(self, X, M=None):
        ...
        log of the joint probability, i.e.
        ...
        M_not_none = np.ones(X.shape, dtype=int) if M is None else M

        ######################## Your code here #########################

        log_p_x_given_z = np.matmul(np.multiply(X, M_not_none), np.transpose(np.log(self.params.theta))) + \
                          np.matmul(1. - np.multiply(X, M_not_none), np.transpose(np.log(1. - self.params.theta)))

        log_p_z = np.log(self.params.pi)

        log_p_x = np.log(np.sum(np.matmul(log_p_x_given_z, np.transpose(log_p_z))))

        log_p_z_x = log_p_x_given_z + log_p_z - log_p_x
    ...
    def posterior_predictive_means(self, X, M):
        ...
        return np.matmul(post, self.params.theta)
